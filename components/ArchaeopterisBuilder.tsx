...
const QUICK_PROMPTS: { label: string; prompt: string; stack: Stack }[] = [
  ...
  { label: 'Admin Sidebar Nav',     prompt: 'Admin sidebar navigation with icons, nested menu, dark theme and collapsible sections',    stack: 'react' },
  { label: 'Pricing Table Fintech', prompt: 'Fintech pricing table with tiers, feature comparison, dark luxury theme',                  stack: 'react' },
]
...
async function generateProject(prompt: string, stack: Stack): Promise<any> {
  const systemPrompts: Record<Stack, string> = {
    react: [
      ...
      'DO NOT generate: package.json, vite.config.js, postcss.config.js, tailwind.config.js, index.html',
    ].join('\n'),
    nextjs: [
      'You are an elite fullstack engineer for Archaeopteris LLC.',
      'Generate a Next.js 14 App Router project as JSON. Response must be ONLY valid JSON, no markdown, no backticks.',
      'Schema: { "name": string, "stack": "nextjs", "files": { "app/page.tsx": string, ... } }',
      '- Use App Router (app/ directory), Tailwind, shadcn/ui',
      '- API routes under app/api/, max 10 files',
      'DO NOT generate: next.config.js, package.json, tailwind.config.js',
    ].join('\n'),
    express: [
      'You are an elite fullstack engineer for Archaeopteris LLC.',
      'Generate an Express + React project as JSON. Response must be ONLY valid JSON.',
      'Schema: { "name": string, "stack": "express", "files": { "server/index.js": string, "src/App.jsx": string, ... } }',
      '- Backend under server/ (Express), frontend under src/ (React + Tailwind)',
      '- server/index.js runs on port 4000, CORS enabled',
      '- Max 10 files, NO package.json',
    ].join('\n'),
    hono: [
      'You are an elite fullstack engineer for Archaeopteris LLC.',
      'Generate a Hono + React project as JSON. Response must be ONLY valid JSON.',
      'Schema: { "name": string, "stack": "hono", "files": { "server/index.js": string, "src/App.jsx": string, ... } }',
      '- Backend under server/ using Hono on port 4000, CORS via hono/cors',
      '- Frontend under src/ (React + Tailwind), max 10 files',
    ].join('\n'),
  }
...
const wcFiles = {
  ...
  'index.html': {
    file: {
      contents: `<!DOCTYPE html><html><head><meta charset="utf-8"/></head><body><div id="root"></div><script type="module" src="/src/main.jsx"></script></body></html>`,
    },
  },
  'vite.config.js': {
    file: {
  ...
  'tailwind.config.js': {
    file: {
      contents: `export default { content: ['./src/**/*.{js,jsx}', './index.html'], theme: { extend: {} }, plugins: [] }`,
    },
  },
  src: {
    directory: {
      ...
      'App.jsx': {
        file: {
          contents: `export default function App() { return <div className='min-h-screen bg-gray-950 flex items-center justify-center'><div className='text-emerald-400 text-4xl font-bold'>Archaeopteris</div></div> }`,
        },
      },
      'index.css': {
        file: {
      ...
      components: {
        directory: {
          ui: {
            directory: {
              'button.jsx': { file: { contents: `import{clsx}from'clsx';import{twMerge}from'tailwind-merge';const cn=(...c)=>twMerge(clsx(c));const variants={default:'bg-emerald-500 text-black hover:bg-emerald-400',destructive:'bg-red-500 text-white hover:bg-red-400',outline:'border border-gray-700 hover:bg-gray-800 text-gray-100',secondary:'bg-gray-800 text-gray-100 hover:bg-gray-700',ghost:'hover:bg-gray-800 text-gray-100',link:'text-emerald-400 underline-offset-4 hover:underline'};const sizes={default:'h-9 px-4 py-2',sm:'h-7 px-3 text-xs',lg:'h-11 px-8',icon:'h-9 w-9'};export function Button({className,variant='default',size='default',...props}){return<button className={cn('inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none disabled:opacity-50',variants[variant],sizes[size],className)}{...props}/>}` } },
              'card.jsx': { file: { contents: `import{clsx}from'clsx';import{twMerge}from'tailwind-merge';const cn=(...c)=>twMerge(clsx(c));export function Card({className,...props}){return<div className={cn('rounded-xl border border-gray-800 bg-gray-900 text-gray-100',className)}{...props}/>}export function CardHeader({className,...props}){return<div className={cn('flex flex-col space-y-1.5 p-4',className)}{...props}/>}export function CardTitle({className,...props}){return<h3 className={cn('font-semibold leading-none tracking-tight',className)}{...props}/>}export function CardContent({className,...props}){return<div className={cn('p-4 pt-0',className)}{...props}/>}export function CardFooter({className,...props}){return<div className={cn('flex items-center p-4 pt-0',className)}{...props}/>}` } },
              'badge.jsx': { file: { contents: `import{clsx}from'clsx';import{twMerge}from'tailwind-merge';const cn=(...c)=>twMerge(clsx(c));const variants={default:'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30',secondary:'bg-gray-800 text-gray-400 border border-gray-700',destructive:'bg-red-500/20 text-red-400 border border-red-500/30',outline:'border border-gray-700 text-gray-300',high:'bg-red-500/20 text-red-400 border border-red-500/30',medium:'bg-amber-500/20 text-amber-400 border border-amber-500/30',low:'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30',crypto:'bg-blue-500/20 text-blue-400 border border-blue-500/30',markets:'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30',forex:'bg-purple-500/20 text-purple-400 border border-purple-500/30'};export function Badge({className,variant='default',...props}){return<span className={cn('inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium',variants[variant]||variants.default,className)}{...props}/>}` } },
              'input.jsx': { file: { contents: `import{clsx}from'clsx';import{twMerge}from'tailwind-merge';const cn=(...c)=>twMerge(clsx(c));export function Input({className,...props}){return<input className={cn('flex h-9 w-full rounded-md border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-gray-100 placeholder:text-gray-500 focus:outline-none focus:ring-1 focus:ring-emerald-500',className)}{...props}/>}` } },
              'separator.jsx': { file: { contents: `import{clsx}from'clsx';import{twMerge}from'tailwind-merge';const cn=(...c)=>twMerge(clsx(c));export function Separator({className,orientation='horizontal',...props}){return<div className={cn('shrink-0 bg-gray-800',orientation==='horizontal'?'h-px w-full':'h-full w-px',className)}{...props}/>}` } },
              'scroll-area.jsx': { file: { contents: `import{clsx}from'clsx';import{twMerge}from'tailwind-merge';const cn=(...c)=>twMerge(clsx(c));export function ScrollArea({className,children,...props}){return<div className={cn('overflow-auto',className)}{...props}>{children}</div>}` } },
              'progress.jsx': { file: { contents: `import{clsx}from'clsx';import{twMerge}from'tailwind-merge';const cn=(...c)=>twMerge(clsx(c));export function Progress({value=0,className}){return<div className={cn('relative h-2 w-full overflow-hidden rounded-full bg-gray-800',className)}><div className='h-full bg-emerald-500 transition-all duration-300' style={{width:\`\${Math.min(100,Math.max(0,value))}%\`}}/></div>}` } },
              ...
              'select.jsx': { file: { contents: `import{clsx}from'clsx';import{twMerge}from'tailwind-merge';const cn=(...c)=>twMerge(clsx(c));export function Select({className,children,...props}){return<select className={cn('flex h-9 w-full rounded-md border border-gray-700 bg-gray-800 px-3 py-1 text-sm text-gray-100 focus:outline-none focus:ring-1 focus:ring-emerald-500',className)}{...props}>{children}</select>}export function SelectItem({value,children}){return<option value={value}>{children}</option>}` } },
      ...
      lib: {
        directory: {
          'utils.js': { file: { contents: `import{clsx}from'clsx';import{twMerge}from'tailwind-merge';export function cn(...inputs){return twMerge(clsx(inputs))}` } },
        },
      },
    },
  },
...
export default function ArchaeopterisBuilder() {
  ...
  const handleGenerate = async (overridePrompt?: string, overrideStack?: Stack) => {
    ...
    try {
      const parsed = await generateProject(p, s)
  ...
  return (
    <div style={{
      fontFamily: 'monospace', background: '#080c10', color: '#e2e8f0',
        ...
        <div style={{
          ...
          <div style={{ padding: 14, borderBottom: '1px solid #1a2535', flexShrink: 0 }}>
            ...
            <textarea
              ...
              placeholder="e.g. SaaS dashboard with analytics..."
              style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, margin: '8px 0' }}>
              {STACKS.map(s => (
                <button key={s.id} onClick={() => setSelectedStack(s.id)} style={{
                  ...
                  fontSize: 10, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left',
                }}>
          ...
          <div style={{ padding: 14, borderBottom: '1px solid #1a2535', flexShrink: 0 }}>
            ...
            {!showTemplates
              ? QUICK_PROMPTS.slice(0, 5).map(q => (
                <button key={q.label} onClick={() => handleGenerate(q.prompt, q.stack)} style={btnBase}>
              ...
              : QUICK_PROMPTS.map(q => (
                <button key={q.label} onClick={() => handleGenerate(q.prompt, q.stack)} style={{
                  ...btnBase, marginBottom: 0,
                  borderColor: STACKS.find(s => s.id === q.stack)?.color + '30',
                }}>
                  <span style={{ color: STACKS.find(s => s.id === q.stack)?.color, fontSize: 9, marginRight: 6 }}>
                    {STACKS.find(s => s.id === q.stack)?.label}
                  </span>
                  {q.label}
                </button>
          ...
          <div style={{ padding: 14, borderBottom: '1px solid #1a2535', flexShrink: 0 }}>
            ...
            <button onClick={() => { navigator.clipboard?.writeText(files[activeFile] || ''); addLog('Copied \u2713') }} style={btnBase}>
              ⎘ Copy File
            </button>
            <button onClick={() => {
              setFiles({ 'App.jsx': STARTER })
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', borderBottom: '1px solid #1a2535', background: '#0a0f1a', flexShrink: 0 }}>
            ...
            <div style={{
              ...
              fontSize: 10, color: STACKS.find(s => s.id === selectedStack)?.color ?? '#10b981', opacity: 0.7,
            }}>
              {STACKS.find(s => s.id === selectedStack)?.label}
            </div>
            {activeTab === 'Code' && (
              <button onClick={async () => {
                const trimmed = (files[activeFile] || '').trim()
                if (trimmed.startsWith('{')) {
                  try {
                    const parsed = JSON.parse(trimmed)
                    const flatted = flattenFiles(parsed.files)
                    setFiles(flatted)
                    setActiveFile(Object.keys(flatted).find(k => k.includes('App.jsx')) || Object.keys(flatted)[0])
                    await wcRef.current?.mountFiles(parsed.files)
          ...
          <div style={{ flex: 1, minHeight: 0, position: 'relative', overflow: 'hidden' }}>
            ...
            {activeTab === 'Code' && (
              <div style={{ position: 'absolute', inset: 0, display: 'flex' }}>
                <div style={{
                  ...
                  borderRight: '1px solid #1a2535', overflowY: 'auto', flexShrink: 0,
                }}>
            ...
            {activeTab === 'Console' && (
              <div style={{
                position: 'absolute', inset: 0, overflow: 'auto', padding: 20,
                fontFamily: 'monospace', fontSize: 12, lineHeight: 1.8,
              }}>
                {logs.map((log, i) => (
                  <div key={i} style={{
                    color: log.includes('Error') ? '#f87171' : log.includes('\u2713') ? '#10b981' : '#4a7090',
                  }}>
