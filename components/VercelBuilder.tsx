...
import VercelContainer, { VercelContainerHandle } from "@/components/vercel-container";
...
export default function ArchaeopterisBuilderE2B() {
  ...
  const vercelRef = useRef<VercelContainerHandle>(null);
...
//await vercelRef.current?.mountFiles(files)

await vercelRef.current?.mountFiles(flatted) // ← dùng flatted

      addLog(`Generated ${Object.keys(generated).length} files ✓`);
      //setFiles(generated);
      const firstFile = Object.keys(generated)[0] ?? "";
      setActiveFile(firstFile);
      //await vercelRef.current?.mountFiles(generated);
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
          ...
          <div style={{
            ...
            {activeTab === "Code" && Object.keys(files).length > 0 && (
              <button
                onClick={() => vercelRef.current?.mountFiles(files)}
                style={{
          ...
          <div style={{ flex: 1, minHeight: 0, position: "relative", overflow: "hidden" }}>
            ...
            <div style={{
              ...
              <VercelContainer ref={vercelRef} style={{ flex: 1, minHeight: 0 }} />
            </div>
