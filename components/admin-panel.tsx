...
export default function AdminPanel() {
  ...
  const handleSave = async () => {
    ...
    const translateRes = await fetch('/api/translate', {
      ...
      body: JSON.stringify({ content, fromLocale: selected.locale })
    })
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="w-full lg:w-80 shrink-0">
            ...
            <div className="space-y-2">
              {posts.length === 0 && (
                <p className="text-sm text-muted-foreground text-center py-8">Chưa có bài viết nào</p>
              )}
              {posts.map(post => (
                <div
                  key={post.id}
                  onClick={() => handleSelect(post)}
                  className={`p-3 rounded-lg border cursor-pointer transition-colors ${selected?.id === post.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'}`}
                >
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className={`text-xs px-1.5 py-0.5 rounded ${post.status === 'published' ? 'bg-primary/20 text-primary' : 'bg-muted text-muted-foreground'}`}>
